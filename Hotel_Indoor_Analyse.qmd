...
| `os`           | Dient zur Interaktion mit dem Betriebssystem, insbesondere zur Arbeit mit Dateistrukturen und Dateipfaden für den Zugriff auf CSV-Dateien.   |
 ...
 `tqdm`  | Zeigt Fortschrittsbalken für iterative Prozesse wie Schleifen oder die Hyperparameter-Suche an und erhöht so die Übersichtlichkeit.     |
...
sns.boxplot(x=combined_df['TA Avg (m)'])
...
```{python}
#| label: univariate analysis - rsrp and rsrq
...
```{python}
#| label: univariate analysis - polygon area
