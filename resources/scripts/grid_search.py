...
grid_search = GridSearchCV(estimator=rf_model, param_grid=param_grid, cv=5, n_jobs=-1, verbose=1) # type: ignore

# Fitting des Modells mit den verschiedenen Hyperparameter-Kombinationen
grid_search.fit(X_train_scaled, y_train) # type: ignore
...
y_pred_best_rf = grid_search.best_estimator_.predict(X_test_scaled) # type: ignore

# Berechnung des Mean Squared Error (MSE) und des R²-Werts für das optimierte Modell
mse_best_rf = mean_squared_error(y_test, y_pred_best_rf) # type: ignore
r2_best_rf = r2_score(y_test, y_pred_best_rf) # type: ignore
